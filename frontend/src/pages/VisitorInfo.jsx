...
function VisitorInfoPage() {
    ...
    if ((state.payment?.status || booking.payment_status) !== "approved") {
        return (
            <section className="card flow-section">
                <h1>Pago pendiente</h1>
                <p>Debes completar y aprobar el pago antes de registrar visitantes.</p>
                <Link to="/pago" className="button button-primary">
                    Ir a pago
                </Link>
            </section>
        );
    ...
    async function handleSubmit(event) {
        ...
        if (visitors.some((visitor) => !visitor.name || !visitor.birth_date || !visitor.id_number)) {
        ...
        setError("");
        try {
            const tickets = await bookingService.registerVisitors(booking.booking_id, visitors);
            setTickets(tickets);
    ...
    return (
        <section className="card flow-section">
            <h1>Informacion de visitantes</h1>
            ...
            <form onSubmit={handleSubmit} className="visitor-form">
                    ...
                    <article className="visitor-card" key={`visitor-${index}`}>
                        ...
                        <label className="field">
                            ...
                            <input
                                ...
                                onChange={(e) => updateVisitor(index, "id_number", e.target.value)}
                ...
                <button type="submit" className="button button-primary" disabled={saving}>
                    {saving ? "Guardando informacion..." : "Finalizar reserva"}
                </button>
            </form>
        </section>
    );
